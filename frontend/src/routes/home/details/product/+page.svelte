<script lang="ts">
	import type { ProductDetails } from "$lib/types";
	import GoBack from "@components/global/GoBack.svelte";

	let product: ProductDetails = {
		name: "Café Ghourmet",
		shortDescription: "Forte",
		description:
			"Um cappuccino é uma bebida de aproximadamente 150 ml (5 oz), com 25 ml de café expresso e 85 ml de leite fresco",
		price: 1.99,
		imageUrl: "data:image/jpeg;base64,",
	};
</script>

<div class="page">
	<header class="product-header">
		<GoBack redirectUrl={"/home"} />
		<h1 class="simple-title">Detalhes do Produto</h1>
	</header>

	<div class="product-card">
		<img
			src={product.imageUrl}
			alt={product.name}
			class="product-image"
		/>

		<h2 class="product-name">{product.name}</h2>
		<p class="product-short-description">{product.shortDescription}</p>

		<hr class="product-divider" />

		<h3 class="product-description-title">Descrição</h3>
		<p class="product-description">
			{product.description}
		</p>
	</div>

	<footer class="product-footer">
		<div class="price-info">
			<span class="price-label">Preço</span>
			<span class="price-value">R$ {product.price.toFixed(2)}</span>
		</div>
		<button class="add-to-cart-button">
			Adicionar ao Carrinho
		</button>
	</footer>
</div>

<style lang="postcss">
	.page {
		@apply min-h-screen flex flex-col bg-light;
	}

	.product-header {
		@apply flex justify-normal items-center mt-auto p-4 bg-white;
	}

	.product-title {
		@apply text-lg font-semibold justify-center ml-10 text-secondary-950;
	}

	.product-card {
		@apply flex flex-col flex-grow max-w-md mx-auto bg-white rounded-lg p-5;
	}

	.product-image {
		@apply h-56 w-full object-cover rounded-lg mb-4;
	}

	.product-name {
		@apply text-xl font-bold text-secondary-950 mb-1;
	}

	.product-short-description {
		@apply text-placeholder mb-3;
	}

	.product-divider {
		@apply border-placeholder mb-3;
	}

	.product-description-title {
		@apply text-lg font-semibold text-secondary mb-2 mt-9;
	}

	.product-description {
		@apply text-secondary mb-4;
	}

	.product-footer {
		@apply flex justify-between items-center mt-auto bg-white px-6 py-4;
	}

	.price-info {
		@apply text-left;
	}

	.price-label {
		@apply block text-placeholder text-sm;
	}

	.price-value {
		@apply text-primary text-xl font-bold;
	}

	.add-to-cart-button {
		@apply bg-primary text-white font-semibold rounded-full px-6 py-2 hover:bg-primary-500 transition duration-speed;
	}
</style>
